...
export function CameraScreen({ navigation }: CameraScreenProps) {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        // Ensure photo locations table exists
  ...
  const saveIdentificationWithLocation = async (scientificName: string, commonName: string, imageUri: string) => {
    try {
      ...
      console.log('Identification saved with location');
    } catch (error) {
  ...
  // Modified takePhoto – saves location after capturing
  ...
  // Modified pickImage – saves location after selecting
...
const styles = StyleSheet.create({
  ...
  warningBox: { backgroundColor: '#fff3e0', padding: 12, borderRadius: 10, marginTop: 16 },
